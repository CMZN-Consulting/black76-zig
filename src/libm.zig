//! libm.zig -- the two transcendental functions the kernel needs, in-repo.
//!
//! IEEE-754 pins the result of + - * / and sqrt to the last bit. It does NOT
//! pin exp and log: those are library code, and two libraries that are both
//! "accurate to 1 ulp" still disagree on the last bit for some inputs. Measured
//! on this kernel's own grid, Zig's bundled exp and glibc's differ on 102 of
//! 1,270 arguments; that alone moved 230 of the 3,516 golden vectors in a
//! line-for-line Python port. So a fixture that depends on the toolchain's
//! libm is really a fixture on the toolchain.
//!
//! This file closes that gap. `exp` and `log` are ports of the algorithms Zig's
//! compiler_rt uses (musl's exp, from FreeBSD's e_exp.c; musl's table-driven
//! log, from ARM optimized-routines), written once over `@Vector(n, f64)` so the
//! same source serves the scalar path (n = 1) and the SIMD batch path. Being
//! the same code, the two paths agree bit-for-bit by construction; being in
//! this repository, the numbers no longer depend on which compiler_rt or libc
//! was linked. `tests/libm_test.zig` holds each function to bit-identity with
//! the compiler_rt originals over tens of millions of samples and every special
//! value.
//!
//! No allocation, no tables built at runtime, no branches on data: special
//! cases are resolved with `@select`, so every lane runs the same instructions.
//! Strict float mode: no contraction, no reassociation, or the bits move.
...
/// Lane-generic exp and log over `@Vector(n, f64)`.
pub fn Lanes(comptime n: comptime_int) type {
    return struct {
        pub const F = @Vector(n, f64);
        pub const U = @Vector(n, u64);
        pub const I = @Vector(n, i64);
        pub const B = @Vector(n, bool);

        pub inline fn splat(x: f64) F {
            return @splat(x);
        }

        inline fn splatU(x: u64) U {
            return @splat(x);
        }

        inline fn splatI(x: i64) I {
            return @splat(x);
        }
        ...
        inline fn fromBits(x: U) F {
            return @bitCast(x);
        }

        /// y * 2^k, correctly rounded, for k in the range exp produces
        /// ([-1075, 1024]). Two exact scalings bracket the final, single
        /// rounding, which is what makes this agree with std.math.ldexp.
        inline fn scalbn(y_: F, k_: I) F {
            var y = y_;
            var k = k_;
            ...
            return y * fromBits(two_k);
        }

        /// e^x. Bit-identical to compiler_rt's f64 exp for every input.
        pub fn exp(x_: F) F {
            ...
            const ux = bits(x_);
            const hx_signed = ux >> @splat(32);
            const negative = (hx_signed >> @splat(31)) != splatU(0);
            const hx = hx_signed & splatU(0x7FFFFFFF);

            // Special lanes, resolved at the end. `is_nan` covers hx > 0x7FF00000.
            const is_nan = x_ != x_;
            const overflow = x_ > splat(709.782712893383973096);
            const underflow = x_ < splat(-745.13321910194110842);
            const tiny = hx <= splatU(0x3E300000); // |x| <= 2^-28: exp(x) = 1 + x
            const special = @select(bool, is_nan, is_nan, @select(bool, overflow, overflow, @select(bool, underflow, underflow, tiny)));
            const any_special = @reduce(.Or, special);

            // Lanes that will be overridden are given a harmless argument so the
            // reduction below never converts an out-of-range float to int.
            ...
            const xx = x * x;
            const c = x - xx * (splat(P1) + xx * (splat(P2) + xx * (splat(P3) + xx * (splat(P4) + xx * splat(P5)))));
            const y = splat(1.0) + (x * c / (splat(2.0) - c) - lo + hi);

            // Uniform branches: skip work every lane would discard. The per-lane
            // arithmetic is the same on both sides of each `if`.
            const k_zero = k == splatI(0);
            const scaled = if (@reduce(.And, k_zero)) y else @select(f64, k_zero, y, scalbn(y, k));
            if (!any_special) return scaled;

            var out = scaled;
            out = @select(f64, tiny, splat(1.0) + x_, out);
            out = @select(f64, underflow, splat(0.0), out);
            out = @select(f64, overflow, splat(std.math.inf(f64)), out);
            out = @select(f64, is_nan, x_, out);
            return out;
        }

        /// ln(x). Bit-identical to compiler_rt's f64 log for every input.
        pub fn log(x_: F) F {
            const poly1 = [_]f64{
                -0x1p-1,
            ...
            const ix_in: I = @bitCast(x_);

            // -- Which lanes want which path. ---------------------------------
            const lo_bound: i64 = @bitCast(@as(f64, 1.0 - 0x1p-4));
            const hi_bound: i64 = @bitCast(@as(f64, 1.0 + 0x1.09p-4));
            const near_one = @select(bool, ix_in >= splatI(lo_bound), ix_in < splatI(hi_bound), @as(B, @splat(false)));
            const top = @as(U, @bitCast(ix_in)) >> @splat(48);
            const special = @select(bool, top < splatU(0x0010), @as(B, @splat(true)), top >= splatU(0x7ff0));
            const any_near = @reduce(.Or, near_one);
            const all_near = @reduce(.And, near_one);
            const any_special = @reduce(.Or, special);

            var out: F = undefined;

            // -- General path: x = 2^k * z, z in [sqrt(2)/2, sqrt(2)) via table. --
            if (!all_near) {
                // Subnormal (and every other special lane, harmlessly): scale by
                // 2^52 and fold the exponent back out of the bits.
                const scaled: I = @as(I, @bitCast(x_ * splat(0x1p52))) -% splatI(52 << 52);
                const ix = if (any_special) @select(i64, special, scaled, ix_in) else ix_in;

                // Wrapping arithmetic: lanes that will be overridden may hold any
                // bit pattern here, and in Debug a plain `-` on them would trap.
                const tmp = ix -% splatI(0x3fe6000000000000);
                const idx: @Vector(n, u7) = @truncate(@as(U, @bitCast(tmp)) >> @splat(45));
                const k: I = @as(@Vector(n, i12), @truncate(@as(I, @bitCast(@as(U, @bitCast(tmp)) >> @splat(52)))));
                const iz = ix -% (k << @splat(52));
                const z: F = @bitCast(iz);

                var invc: [n]f64 = undefined;
                var logc: [n]f64 = undefined;
                var chi: [n]f64 = undefined;
                var clo: [n]f64 = undefined;
                inline for (0..n) |j| {
                    const entry = log_tab[idx[j]];
                    const entry2 = log_tab2[idx[j]];
                    invc[j] = entry.invc;
                    logc[j] = entry.logc;
                    chi[j] = entry2.chi;
                    clo[j] = entry2.clo;
                ...
                const r = (z - @as(F, chi) - @as(F, clo)) * @as(F, invc);
                const kd: F = @floatFromInt(k);
                const w = kd * splat(0x1.62e42fefa3800p-1) + @as(F, logc);
                const hi = w + r;
                const lo = w - hi + r + kd * splat(0x1.ef35793c76730p-45);
                const r2 = r * r;
                out = lo + r2 * splat(poly[0]) + r * r2 * (splat(poly[1]) + r * splat(poly[2]) + r2 * (splat(poly[3]) + r * splat(poly[4]))) + hi;
            }

            // -- Near 1: a plain polynomial in r = x - 1, no table. ---------
            if (any_near) {
                const r = x_ - splat(1.0);
                const r2 = r * r;
                const r3 = r * r2;
                const y = r3 * (splat(poly1[1]) + r * splat(poly1[2]) + r2 * splat(poly1[3]) +
                    r3 * (splat(poly1[4]) + r * splat(poly1[5]) + r2 * splat(poly1[6]) +
                        r3 * (splat(poly1[7]) + r * splat(poly1[8]) + r2 * splat(poly1[9]) + r3 * splat(poly1[10]))));
                var w = r * splat(0x1p27);
                const rhi = r + w - w;
                const rlo = r - rhi;
                w = rhi * rhi * splat(poly1[0]);
                const hi = r + w;
                const lo = r - hi + w + splat(poly1[0]) * rlo * (rhi + r);
                const near = y + lo + hi;
                const exactly_one = ix_in == splatI(@as(i64, @bitCast(@as(f64, 1.0))));
                const near_or_zero = @select(f64, exactly_one, splat(0.0), near);
                out = if (all_near) near_or_zero else @select(f64, near_one, near_or_zero, out);
            }

            if (!any_special) return out;

            // -- Special lanes: zero, negative, inf, nan. -------------------------
            // Later selects override earlier ones, so this runs in REVERSE of the
            // scalar code's check order: zero is checked before inf before
            // negative/NaN there, so zero must land last here. (+inf and -0.0
            // both also satisfy the negative-or-NaN test; the order settles them.)
            const is_zero = (ix_in << @splat(1)) == splatI(0);
            const is_inf = ix_in == splatI(@as(i64, @bitCast(std.math.inf(f64))));
            const is_neg_or_nan = @select(bool, (top & splatU(0x8000)) != splatU(0), @as(B, @splat(true)), (top & splatU(0x7ff0)) == splatU(0x7ff0));
            // (x - x) * inf rather than a canonical NaN: an invalid operation
            // yields the hardware's own default NaN for negative x and propagates
            // the payload of a NaN x, exactly as the scalar original's (x - x) / 0.
            out = @select(f64, is_neg_or_nan, (x_ - x_) * splat(std.math.inf(f64)), out);
            out = @select(f64, is_inf, x_, out);
            out = @select(f64, is_zero, splat(-std.math.inf(f64)), out);
            return out;
        }
    };
}

const log_tab = [128]struct { invc: f64, logc: f64 }{
...
const log_tab2 = [128]struct { chi: f64, clo: f64 }{
...
test "Lanes(1) exp and log match compiler_rt on the values the kernel sees most" {
    const L = Lanes(1);
    for ([_]f64{ 0.0, -0.0, 1.0, -1.0, 0.5, -0.5, 2.0, -20.0, -100.0, -700.0, -708.5, -740.0, 700.0, 709.7, 1e-10, -1e-10 }) |x| {
        try std.testing.expectEqual(@as(u64, @bitCast(@exp(x))), @as(u64, @bitCast(L.exp(L.splat(x))[0])));
    }
    for ([_]f64{ 1.0, 0.5, 2.0, 1.0 - 0x1p-5, 1.0 + 0x1p-5, 0.99, 1.01, 1e-300, 1e300, 5e-324, 0x1p-1022, 1000.0, 3.0 }) |x| {
        try std.testing.expectEqual(@as(u64, @bitCast(@log(x))), @as(u64, @bitCast(L.log(L.splat(x))[0])));
    }
}

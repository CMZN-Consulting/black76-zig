...
fn writeHeader(w: *std.Io.Writer) !void {
    ...
    try w.print(
        \\{{"header":1,"schema":"black76-golden/3","zig_version":"{s}","target":"{s}-{s}-{s}","cpu":"{s}","optimize":"{s}","source_sha256":"{x}","source_bytes":{d},"vector_count":{d},"r":"0.0 hardcoded","theta_units":"per year","ttm_units":"years"}}
        \\
    , .{
